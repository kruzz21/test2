/*
  ...
  1. Security Updates
    - Clean up all existing RLS policies on all tables
    - Create new policies using Supabase's built-in JWT authentication
    - Use auth.jwt() ->> 'app_metadata' ->> 'is_admin' = 'true' for admin access
  ...
  3. Policy Structure
    - Admin policies: FOR ALL with app_metadata.is_admin check
    - Public insert policies: FOR INSERT with true check
    - Public read policies: FOR SELECT with appropriate conditions
*/
